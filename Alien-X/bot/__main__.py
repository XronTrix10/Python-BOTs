...
def start(update, context):
    buttons = ButtonMaker()
    buttons.buildbutton("Contact Owner", "https://t.me/BEnTeN009")
    reply_markup = buttons.build_menu(1)
    if CustomFilters.authorized_user(update) or CustomFilters.authorized_chat(update):
        start_string = f'''
Hey there 🤖👋🏻
I can mirror all your links to Google Drive or to Telegram !
Just send me any direct link 😎
Type /{BotCommands.HelpCommand} to get a list of available commands
'''
        sendMarkup(start_string, context.bot, update.message, reply_markup)
    else:
        sendMarkup('You are not allowed to use me 😡', context.bot, update.message, reply_markup)
